...
# Restore advanced computer searches into Jamf Pro from backup XML files.
# Author: Karthikeyan Marappan
#
# Companion to delete-all-advanced-computer-searches.sh. That script saves each
# search as <prod|dev>-<name>-<id>.xml (IDs removed) before deleting it. This
# script creates those searches again.
#
# *** A restored search is created with the same criteria and display fields.
# *** No Mac is changed. Review the dry run first.
#
# Key Functionalities:
...
#   backup, so this only matters for raw exports), then creates the search
#   with POST /JSSResource/advancedcomputersearches/id/0 (Classic API).
# - Lists the searches that already exist (GET /JSSResource/advancedcomputersearches)
#   and skips a backup whose search name already exists, so nothing is
...
#   RESTORE after showing how many searches it will create. A non-interactive
...
SCRIPT_NAME="restore-advanced-computer-searches.sh"
...
SCRIPT_LOG_FILE="$HOME/Library/Logs/jamf_restore_advanced_computer_searches.log"
...
if [[ "$JAMF_ENV" == "prod" ]]; then
	{
		...
		if [[ "$DRY_RUN" == "yes" ]]; then
			...
			echo "#  DRY_RUN=no: this WILL CREATE up to ${#restoreFile[@]} advanced computer search(es) in"
			echo "#  PRODUCTION."
		fi
		echo "################################################################"
	} >&2
...
if [[ "$DRY_RUN" == "no" ]]; then
	{
		echo
		echo "================================================================"
		echo " DISCLAIMER: THIS SCRIPT CREATES ADVANCED COMPUTER SEARCHES"
		echo
		echo " - It creates an advanced computer search in Jamf Pro from each"
		echo "   backup file it is given (${#restoreFile[@]} file(s) found) on:"
		echo "   $JAMF_URL"
		echo " - A search whose name already exists is skipped, not changed."
		echo " - A backup has no IDs, so the restored search gets a new ID and"
		echo "   the site is set by name. Criteria that refer to a group, site or"
		echo "   Extension Attribute that no longer exists may make Jamf Pro"
		echo "   reject the search. Check each restored search."
		echo " - A restored search runs against the current Macs, so its results"
		echo "   are not the saved ones (a backup holds no results)."
...
printSummary() {
	log "---------------------------------------"
	log "Summary for advanced computer search restore"
	log "Backup files found: $countFiles"
	log "Skipped (name already exists or repeated in this run): $countSkipped"
	if [[ ${#skippedSearch[@]} -gt 0 ]]; then
		printf "%s\n" "${skippedSearch[@]}" | tee -a "$logFile"
	fi
	log "---------------------------------------"
	if [[ "$DRY_RUN" == "yes" ]]; then
	...
	if [[ ${#restoredSearch[@]} -gt 0 ]]; then
		printf "%s\n" "${restoredSearch[@]}" | tee -a "$logFile"
	fi
	log "---------------------------------------"
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/jamf-restore-searches.XXXXXX")"
...
# create the search twice). A 401 means the token expired, so it gets a new
...
stopForPrivilege() {
	runFailureReason="Forbidden (403) while $1. The API Role needs Read and Create for Advanced Computer Searches; remaining searches were not attempted."
	log "Error: Forbidden (403) while $1. The API Role lacks a required privilege."
	exit 1
}

# Names of the searches that exist now (bash 3.2 has no associative arrays).
existingName=()
...
listExisting() {
	...
	case "$http_status" in
		...
		*)
			runFailureReason="Failed to list advanced computer searches (HTTP $http_status). Nothing was created."
	...
	# than treat it as "no searches" and risk creating duplicates.
	...
	log "Found ${#existingName[@]} existing advanced computer search(s) in Jamf Pro."
}

# nameExists NAME: true if a search with exactly this name already exists.
nameExists() {
	local entry
...
# xmlUnescape: decodes the five XML entities on stdin (used for the search name).
xmlUnescape() {
	sed -e 's/&lt;/</g' -e 's/&gt;/>/g' -e 's/&quot;/"/g' -e "s/&apos;/'/g" -e 's/&amp;/\&/g'
}

# prepareFile FILE: writes the XML to send to $POST_FILE (every <id> element
# removed, and any <computers> results block dropped) and sets searchName from
# the file. Returns non-zero if the file is not a usable search. perl (ships
# with macOS) is used for <computers> because it can match across lines.
searchName=""
prepareFile() {
	local text
	searchName=""
	perl -0pe 's#<computers>.*?</computers>##sg' "$1" \
		| sed -E '/^[[:space:]]*<id>[^<]*<\/id>[[:space:]]*$/d; s#<id>[^<]*</id>##g' > "$POST_FILE" || return 1
	grep -q '<advanced_computer_search' "$POST_FILE" && grep -q '</advanced_computer_search>' "$POST_FILE" || return 1
	# The first <name> in the file is the search's own name (it comes before
	# the criteria and display fields, which have names of their own). Bash expansion is used because it takes the FIRST match even
	# when the whole file is on one line.
	text="$(cat "$POST_FILE")"
	[[ "$text" == *"<name>"* ]] || return 1
	text="${text#*<name>}"
	text="${text%%</name>*}"
	searchName="$(printf '%s' "$text" | xmlUnescape)"
	[[ -n "$searchName" ]]
}
...
createSearch() {
	...
	jamfRequest POST "/JSSResource/advancedcomputersearches/id/0" "application/xml" "$POST_FILE"
	case "$http_status" in
		200|201)
			...
			restoredSearch+=("$label (new ID: $newId)")
			log "Restored advanced computer search $label (new ID: $newId)"
			;;
		403)
			countFailure=$((countFailure + 1))
			failureSearch+=("$label (HTTP 403)")
			stopForPrivilege "creating advanced computer search $label"
			;;
		*)
			countFailure=$((countFailure + 1))
			failureSearch+=("$label (HTTP $http_status)")
			log "Failed to restore advanced computer search $label. HTTP code: $http_status"
			;;
	esac
}
...
main() {
	...
	for ((i = 0; i < countFiles; i++)); do
		...
		if ! prepareFile "$file"; then
			...
			name+=("")
			countFailure=$((countFailure + 1))
			failureSearch+=("$label (not a usable search file)")
			log "Not restored: $label is not a usable search file."
		elif nameExists "$searchName"; then
			action+=("exists")
			name+=("$searchName")
			countSkipped=$((countSkipped + 1))
			skippedSearch+=("$searchName ($label)")
			log "Skipping $label: a search named \"$searchName\" already exists (in Jamf Pro, or from an earlier file in this run)."
		else
			action+=("restore")
			name+=("$searchName")
			countToRestore=$((countToRestore + 1))
			# Count this name as taken so a second file with the same search name
	...
	for ((i = 0; i < countFiles; i++)); do
		...
		if [[ "$DRY_RUN" == "yes" ]]; then
			countRestored=$((countRestored + 1))
			restoredSearch+=("$label")
			log "DRY_RUN: would restore advanced computer search $label"
			continue
		fi
		# Rebuild the file to send (prepareFile wrote $POST_FILE for the last file).
		prepareFile "$file" || { countFailure=$((countFailure + 1)); failureSearch+=("$label (could not be prepared)"); continue; }
		createSearch "$label"
	done
