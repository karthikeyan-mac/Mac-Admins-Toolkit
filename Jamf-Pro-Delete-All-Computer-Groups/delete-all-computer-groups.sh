...
# - Before deleting anything on a real run, saves each group's full record
#   (GET /api/v3/computer-groups/smart-groups/{id} or /static-groups/{id}, so
#   smart group criteria and static group members) to a timestamped folder in
#   BACKUP_DIR. A group whose backup fails is not deleted.
...
if [[ "$DRY_RUN" == "no" ]]; then
	{
		...
		echo "   deleting, it saves each group's record to a new folder in:"
		echo "   $BACKUP_DIR"
		echo "   Jamf Pro has no import for that file: use it to rebuild by hand."
...
# backupGroup INDEX: saves the group's full record (smart group criteria, static
# group members) as JSON in the backup folder. Returns non-zero if the record could
# not be saved, so the group is not deleted.
backupGroup() {
	local i="$1" id="${groupId[$1]}" name="${groupName[$1]}" kind="${groupKind[$1]}"
	...
	cp "$RESPONSE_FILE" "$backupPath/$kind-$id.json" || return 1
}
